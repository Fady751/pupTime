"""Shared executor for AI-proposed task actions.

Used by both ``ApproveAIChoiceView`` (HTTP approval) and ``AILoopService``
(WebSocket loop approval) so the create/update/delete logic lives in one place.
"""
...
def execute_action(user, action):
    ...
    if action_name == 'update_TaskTemplate':
        ...
        task_data = TaskSerializer(updated_task, context=_one_month_serializer_context()).data
        return {
            'action_name': action_name,
