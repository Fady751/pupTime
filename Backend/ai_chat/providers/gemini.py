...
class GeminiProvider(BaseAIProvider):
    ...
    def extract_tasks(
        ...
        prompt_instruction = (
            "You split a user's request into the distinct tasks they want to schedule "
            "or manage. Return ONLY a JSON array of short task description strings, in the "
            "order the user mentioned them. If the message contains no actionable tasks "
            "(e.g. it is a question or greeting), return an empty array []. "
        ...
        if audio_bytes and audio_mime_type:
            audio_b64 = base64.b64encode(audio_bytes).decode("utf-8")
            lc_messages.append(HumanMessage(content=[
                {"type": "text", "text": user_message or "Listen to this voice message and extract the tasks."},
                {
                    "type": "audio",
                    "source_type": "base64",
                    "data": audio_b64,
            ...
            lc_messages.append(HumanMessage(content=user_message or ""))
        ...
        return self._parse_task_array(response.content)

    @staticmethod
    def _parse_task_array(content: str) -> List[str]:
        import json
