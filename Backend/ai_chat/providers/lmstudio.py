...
def _raise_provider_error(error: Exception, base_url: str) -> None:
    if isinstance(error, APIConnectionError):
        raise AIProviderError(
            "Cannot connect to LM Studio at "
            f"{base_url}. Start LM Studio local server and confirm the host/port."
        ) from error

    if isinstance(error, APIStatusError):
        status_code = getattr(error, "status_code", None)
        raise AIProviderError(
            f"LM Studio request failed with HTTP {status_code}. "
            "Check that the selected model is loaded and supports chat completions."
        ) from error
...
class LMStudioProvider(BaseAIProvider):
    def __init__(self) -> None:
        model_name = config("LMSTUDIO_MODEL", default="local-model")
        self._base_url = config("LMSTUDIO_BASE_URL", default="http://127.0.0.1:1234/v1")
        api_key = config("LMSTUDIO_API_KEY", default="lm-studio")
        temperature = float(config("LMSTUDIO_TEMPERATURE", default="0.2"))
    ...
    def generate(self, messages: List[ChatMessage]) -> str:
        ...
        try:
            ...
            _raise_provider_error(error, self._base_url)
