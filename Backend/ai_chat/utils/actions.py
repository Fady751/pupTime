...
def _normalize_action_params(params):
    """Coerce action params into a dict and apply title aliases (strict path).

    Raises ValidationError on malformed params — used at approve-time, where a
    bad payload must surface to the user as a 400.
    """
...
def _create_task_template(user, params):
    ...
    return {'action_name': 'create_TaskTemplate', 'task_id': str(task.id), 'task_data': serializer.data}


def _update_task_template(user, params):
    ...
    return {'action_name': 'update_TaskTemplate', 'task_id': str(updated_task.id), 'task_data': task_data}


def _update_task_override(user, params):
    ...
    override.save()
    return {
        'action_name': 'update_TaskOverride',
...
def _delete_task_template(user, params):
...
class ActionSpec:
    ...
    ``schema`` is the pydantic model the LLM sees and that propose-time validation
    runs against; ``execute`` performs the action at approve-time.
    """
    name: str
    schema: type
    execute: Callable
...
ACTION_REGISTRY: dict[str, ActionSpec] = {
    spec.name: spec
    for spec in (
        ActionSpec('create_TaskTemplate', CreateTaskTemplateSchema, _create_task_template),
...
def action_validators() -> dict:
    """Map of action_name -> pydantic schema, for propose-time validation."""
    return {name: spec.schema for name, spec in ACTION_REGISTRY.items()}
